...
def verify_workspace() -> None:
    """Log the current GlamUp file layout and token state."""

    root = Path(__file__).resolve().parent
    LOGGER.info("GlamUp root: %s", root)

    checkpoints = {
        "UI shell": root / "glamup_ui.py",
        "LLM outfits": root / "glamup_llm_outfits.py",
        "Outfit generator entrypoint": root / "outfit_generator.py",
        "Requirements": root / "requirements.txt",
    }

    for label, path in checkpoints.items():
        if path.exists():
            LOGGER.info("✔ %s present at %s", label, path)
        else:
            LOGGER.warning("✖ %s missing at %s", label, path)

    if HF_API_TOKEN:
        LOGGER.info("HF_API_TOKEN loaded for authenticated hub downloads.")
    else:  # pragma: no cover - environment dependent
        LOGGER.warning("HF_API_TOKEN not set; using anonymous access.")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    parser.add_argument("--base", dest="base_model_image_path", required=False, help="Path to the base model image")
    ...
    parser.add_argument("--output", dest="output_path", required=False, help="Output path for the final outfit card")
    ...
    parser.add_argument("--verify-layout", action="store_true", help="Log expected GlamUp files and exit")
    ...
    if not args.base or not args.output:
        parser.error("--base and --output are required unless using --verify-layout")
